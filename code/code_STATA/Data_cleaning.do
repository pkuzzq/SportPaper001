...
****这个环节在已经进入实证环节后，方便调整不显著的变量用的，关键参数的宏
    global  max_expd  = 0.93   //房屋支出的上限
    global  min_expd  = 0.33   //房屋支出的下限
    global  low_upper = 40     //最低收入的上限
    global  low_down  = 2      //最低收入的上限


** 参数组合 
    *-  KM符合   *-  绝大多数参数模型符合  *-  COX模型符合
